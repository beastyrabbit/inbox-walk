...
import { buildReviewBundlesFromPartition } from './bundles.ts'
import { demoEmails } from './demo.ts'

describe('restored bundle groups', () => {
  it('preserves group order, source order and chronological timelines', async () => {
    const emails = demoEmails.slice(0, 3)
    const run = await buildReviewBundlesFromPartition('fixture', emails, async (input) => ({
      standaloneEmailIds: input.emails.map((email) => email.id),
      stories: [],
    }))
